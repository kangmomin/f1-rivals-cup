...
import { FinanceStats, WeeklyFlow, TeamWeeklyFlow } from '../../services/finance'

interface FinanceChartProps {
  stats?: FinanceStats
  accountWeeklyFlow?: WeeklyFlow[]  // 계좌별 주별 통계 (제공 시 리그 전체 대신 사용)
  showTeamBalances?: boolean  // 팀별 잔액 차트 표시 여부 (기본: true)
  teamWeeklyFlows?: TeamWeeklyFlow[]  // 팀별 주별 자금 흐름 (다중 라인 차트용)
}

export default function FinanceChart({ stats, accountWeeklyFlow, showTeamBalances = true, teamWeeklyFlows }: FinanceChartProps) {
  ...
  const processTeamWeeklyFlows = () => {
    const flows = teamWeeklyFlows ?? stats?.team_weekly_flows ?? []
    if (flows.length === 0) return { data: [], teams: [] }

    // 모든 주차를 수집
    const allWeeks = new Set<string>()
    flows.forEach(team => {
      team.flows.forEach(flow => allWeeks.add(flow.week))
    })
  ...
  const { data: teamFlowData, teams: teamList } = processTeamWeeklyFlows()
